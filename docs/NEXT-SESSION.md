...
2. **Superseded-migration guards hardened against `psql`** — all five now wrap
   ...
   left to run.
3. **Bridge fixed, verified, and installed as v0.2.0.** It holds live sockets to
   the roaster and to Supabase. Old copies trashed; `/Applications/RoastLogs
   Bridge.app` is now the single install and the Dock pin resolves to it.
   **Root cause never proven — read item #2 before assuming it is gone for good.**
